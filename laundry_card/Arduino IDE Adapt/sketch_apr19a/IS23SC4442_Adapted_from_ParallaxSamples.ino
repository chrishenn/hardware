...
//     Note: Being that I locked my first card ... and then found out even the PBasic 2.5 and Propeller Demos would
//     automatically lock after 3 card inserts
...
//           But for testing ... locking up the card (so you can't write values) shouldn't be possible with this code
//           (but no guarantees).
...
#define IO  33 // Input/Output Pin
#define CLK 28 // Clock Pin

#define RST 37 // Reset Pin
#define VCC 53 // Card Detect Pin
...
#define WritePass 0x33 // Compare Verification Data
...
void setup() {
    ...
    Serial.println("Note: 32321 Card Expected (per original code)");
...
void Authenticate() {
    ...
    uint8_t Data = 0x03; // Just make sure we never lock the Card (came as 0x03, but our card has 4 reads b4 lock)

    uint8_t Password1 = 0x60; // 1st byte of PSC
    uint8_t Password2 = 0x57; // 2nd byte of PSC
    uint8_t Password3 = 0x34; // 3rd byte of PSC
    ...
    Serial.print(errCounter, HEX);

    Send_Command(UpdateSecurity, 0x00, Data);

    Send_Command(WritePass, 0x01, Password1);

    Send_Command(WritePass, 0x02, Password2);

    Send_Command(WritePass, 0x03, Password3);
