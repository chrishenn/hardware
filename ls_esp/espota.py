...
def authenticate(
    ...
    if use_old_protocol:
        ...
        derived_key = hashlib.pbkdf2_hmac(
            "sha256", password_hash.encode(), salt.encode(), 10000
        )
...
def serve(  # noqa: C901
    ...
    success, data, error = send_invitation_and_get_auth_challenge(
        remote_addr, remote_port, message
    )
    if not success:
        logging.error(error)
        return 1
    ...
    if data != "OK":
        if data.startswith("AUTH"):
            ...
            if nonce_length == 32:
                ...
                logging.warning(
                    "WARNING: Device is using old MD5 authentication protocol (pre-3.3.1)"
                )
                logging.warning(
                    "Please update to ESP32 Arduino Core 3.3.1+ for improved security."
                )
                logging.warning(
                    "======================================================================"
                )
                ...
                if md5_target:
                    ...
                    sys.stderr.write(
                        "Authenticating (SHA256 protocol with MD5 password)..."
                    )
                    sys.stderr.flush()
                    auth_success, auth_error = authenticate(
                        remote_addr,
                    ...
                    if not auth_success:
                        ...
                        if not success:
                            sys.stderr.write("FAIL\n")
                            logging.error(
                                "Failed to get new challenge for MD5 retry: %s", error
                            )
                            return 1
                        ...
                        if not data.startswith("AUTH"):
                            sys.stderr.write("FAIL\n")
                            logging.error(
                                "Expected AUTH challenge for MD5 retry, got: %s", data
                            )
                            return 1
                        ...
                        if auth_success:
                            ...
                            logging.warning(
                                "WARNING: Device authenticated with MD5 password hash (deprecated)"
                            )
                            logging.warning(
                ...
                logging.error(
                    "Invalid nonce length: %d (expected 32 or 64)", nonce_length
                )
                return 1
        else:
        ...
        with open(filename, "rb") as f:
            ...
            while count < 10:  # Increased from 5 to 10 attempts
                ...
                try:
                    ...
                    logging.debug(
                        "Error receiving result (attempt %d/10): %s", count, str(e)
                    )
            ...
            if received_any_response:
                ...
                logging.warning(
                    "Device might be rebooting to apply firmware - this is normal."
                )
                connection.close()
                ...
                logging.error(
                    "This could indicate device reboot (normal) or network issues"
                )
                connection.close()
                return 1
