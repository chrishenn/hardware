...
Usage:
    python sign.py --bin firmware.bin --key private_key.pem --out firmware_signed.bin
...
def generate_rsa_key(key_size, output_file):
    ...
    print(
        "Extract the public key with: python bin_signing.py --extract-pubkey",
        output_file,
    )
...
def sign_binary(binary_file, key_file, output_file, hash_algo="sha256"):
    ...
    if hash_algo not in hash_algos:
        print(
            f"Error: Unsupported hash algorithm. Supported: {', '.join(hash_algos.keys())}"
        )
        sys.exit(1)
    ...
    print(
        f"Signed binary size: {signed_size} bytes (firmware: {len(binary_data)}, signature: {len(padded_signature)})"
    )
    print(f"\nKey type: {key_type}")
...
Examples:
  ...
  Verify signed firmware:
    ...
    parser.add_argument(
        "--pubkey", metavar="FILE", help="Public key file for verification (PEM format)"
    )
    ...
    if args.generate_key:
        ...
        if key_type.startswith("rsa-"):
            ...
            print(
                "Error: Invalid key type. Supported: rsa-2048, rsa-3072, rsa-4096, ecdsa-p256, ecdsa-p384"
            )
            sys.exit(1)
